...
describe('Brainy Batch Operations - Fixed', () => {
  ...
  describe('relateMany - Batch Relationship Creation', () => {
    ...
    it('should create multiple relationships at once', async () => {
      ...
      const result = await brain.relateMany({ relations: relationships })
    ...
    it('should handle different relationship types', async () => {
      const relationships = [
        ...
        { from: entities[3], to: entities[4], type: VerbType.Competes }
      ]
    ...
    it('should handle large batch of relationships', async () => {
      ...
      const relationships = manyPeopleResult.successful.map(person => ({
        from: person,
        to: company,
  ...
  describe('Error Handling in Batch Operations', () => {
    it('should handle empty batches gracefully', async () => {
      ...
      const relateResult = await brain.relateMany({ relations: [] })
      expect(relateResult.successful).toHaveLength(0)
    })
  })
})